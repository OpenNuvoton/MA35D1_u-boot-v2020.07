                               ...
                               const struct spi_mem_op *op)
{
	...
	if (op->data.nbytes) {
		if (op->data.dir == SPI_MEM_DATA_OUT) {
			...
			if ((op->cmd.opcode == QUAD_READ_0x6B) && ((op->data.nbytes % 4) == 0)) {
				...
				ma35d1_qspi_write((ma35d1_qspi_read(nq, CTL) & ~(DWIDTH_MASK)) | SPI_REORDER, nq, CTL);
				for (i = 0; i < op->data.nbytes; ) {
					if ((ma35d1_qspi_read(nq, STATUS) & TXFULL) == 0) { //TX not FULL
						ma35d1_qspi_write(0, nq, TX);
						i += 4;
					}
					if ((ma35d1_qspi_read(nq, STATUS) & RXEMPTY) == 0) { //RX not EMPTY
						*(unsigned int*)rx = ma35d1_qspi_read(nq, RX);
						rx += 4;
						j += 4;
					}
				}
	...
	ma35d1_qspi_write((ma35d1_qspi_read(nq, CTL) & ~(SPI_QUAD_EN | SPI_DIR_2QM | DWIDTH_MASK | SPI_REORDER)) | 0x800, nq, CTL);
