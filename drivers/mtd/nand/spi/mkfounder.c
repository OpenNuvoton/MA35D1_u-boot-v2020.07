...
static int mkfd_spinand_detect(struct spinand_device *spinand)
{
	 ...
	 */
	if (id[1] != SPINAND_MFR_MK)
		return 0;

	ret = spinand_match_and_init(spinand, mkfd_spinand_table,
				     ARRAY_SIZE(mkfd_spinand_table), id[2]);
	if (ret)
		return ret;
